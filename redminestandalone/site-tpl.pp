	...
	exec { "run_rm_rminstall.sh":
		command => "chown -R www:www /usr/local/www/redmine && \
cd /usr/local/www/redmine && bundle exec rake generate_secret_token && \
cd /usr/local/www/redmine && env RAILS_ENV=production bundle exec rake db:migrate && \
cd /usr/local/www/redmine && env RAILS_ENV=production REDMINE_LANG=en bundle exec rake redmine:load_default_data &&
touch /var/log/rminstall.done > /var/log/rminstall.log 2>&1",
		onlyif => "test ! -f /var/log/rminstall.done && grep -q cix_redmine /usr/local/www/redmine/config/database.yml",
